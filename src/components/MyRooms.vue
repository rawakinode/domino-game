...
onMounted(async () => {
    try {
        ...
        for (let i = 0; i < g.length; i++) {
            ...
            if(a.player2.toLowerCase() != "0x0000000000000000000000000000000000000000") {
                status = "In Game"
            }

            let setplayer = "";
            if (a.player1.toLowerCase() == account.value) {
                if (a.player2 == "0x0000000000000000000000000000000000000000") {
                    setplayer = "-"
                } else {
                    setplayer = a.player2.toLowerCase();
                }
            } else {
                setplayer = a.player1.toLowerCase();
            }

            if (a.completed == false) {
                h.push({
                    id: idx, address: setplayer, amount: s, status: status, block: Number(a.createdAtBlock)
                });
            }
        }
...
<template>
    ...
    <div class="fullscreen-box">
        <div class="tengah" v-if="isFound == false">Game not found. <br> Try to create one or join available game.</div>
        <div v-if="visible == 'main'" v-for="(player, index) in players" :key="index" class="box">
            ...
            <button class="button2" style="float: right; background-color:rgb(233, 206, 0);">ID: {{ player.id }}</button>
            <button class="button2" style="float: right; background-color: rgb(95, 95, 95);">{{ player.status }}</button>
